/*
    Add this syntax on process before header page filter, and javascript trigger when filtering
    BEGIN
        BJKT_PNL_FILTER_CTX_PKG.SET_USER_CONTEXT(
            p_username    => :APP_USER,
            p_start_date  => SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_START_DATE'),
            p_end_date    => SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_END_DATE'),
            p_kode_konsol => SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_KODE_KONSOL'),
            p_kode_cabang => SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_KODE_CABANG')
        );
    END;
*/

CREATE OR REPLACE CONTEXT BJKT_PNL_FILTER_CTX USING BJKT_PNL_FILTER_CTX_PKG ACCESSED GLOBALLY;
/
CREATE OR REPLACE CONTEXT BJKT_PNL_FILTER_SCORECARD_CTX USING BJKT_PNL_FILTER_CTX_PKG ACCESSED GLOBALLY;
/

CREATE OR REPLACE PACKAGE BJKT_PNL_FILTER_CTX_PKG AS
    PROCEDURE SET_USER_CONTEXT(
        p_username    IN VARCHAR2,
        p_feature     IN VARCHAR2,
        p_start_date  IN VARCHAR2 DEFAULT NULL,
        p_end_date    IN VARCHAR2 DEFAULT NULL,
        p_kode_konsol IN VARCHAR2 DEFAULT NULL,
        p_kode_cabang IN VARCHAR2 DEFAULT NULL
    );
END;
/

CREATE OR REPLACE PACKAGE BODY BJKT_PNL_FILTER_CTX_PKG AS
    PROCEDURE SET_USER_CONTEXT(
        p_username    IN VARCHAR2,
        p_feature     IN VARCHAR2,
        p_start_date  IN VARCHAR2 DEFAULT NULL,
    ...
    BEGIN
        IF p_feature IS NULL THEN
            RAISE_APPLICATION_ERROR(-20001, 'feature cant be empty');
        ELSIF p_feature NOT IN ('SUMMARY', 'SCORECARD') THEN
            RAISE_APPLICATION_ERROR(-20001, 'feature undefined');
        END IF;

        DBMS_SESSION.SET_CONTEXT('BJKT_PNL_FILTER_CTX', p_feature || '_USERNAME',     p_username,    v_client_id);
        DBMS_SESSION.SET_CONTEXT('BJKT_PNL_FILTER_CTX', p_feature || '_START_DATE',   p_start_date,  v_client_id);
        DBMS_SESSION.SET_CONTEXT('BJKT_PNL_FILTER_CTX', p_feature || '_END_DATE',     p_end_date,    v_client_id);
        DBMS_SESSION.SET_CONTEXT('BJKT_PNL_FILTER_CTX', p_feature || '_KODE_KONSOL',  p_kode_konsol, v_client_id);
        DBMS_SESSION.SET_CONTEXT('BJKT_PNL_FILTER_CTX', p_feature || '_KODE_CABANG',  p_kode_cabang, v_client_id);
    END;
END;
/

-- Table View Summary PnL
CREATE OR REPLACE VIEW BJKT_PNL_SUMMARY_V2_V AS
WITH 
cre AS (
    SELECT
        "kode_cabang",
    ...
    WHERE "periode" >= TO_DATE(SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_START_DATE'), 'DD-MONTH-YYYY', 'NLS_DATE_LANGUAGE=ENGLISH')
      AND "periode" <  TO_DATE(SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_END_DATE'), 'DD-MONTH-YYYY', 'NLS_DATE_LANGUAGE=ENGLISH') + 1
      AND "kode_konsol" = NVL(SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_KODE_KONSOL'), "kode_konsol")
      AND "kode_cabang" = NVL(SYS_CONTEXT('BJKT_PNL_FILTER_CTX', 'SUMMARY_KODE_CABANG'), "kode_cabang")
