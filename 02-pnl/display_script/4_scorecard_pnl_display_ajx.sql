...
BEGIN
    ...
    FOR r IN (
        ...
        q_mapping ("column_name", "column_desc", "group_number", "is_header", "is_lines", "column_number") AS (
            ...
            SELECT 'FBI_KER_PIHAK_LAIN',          'Kerjasama Pihak Lain (komisi agen, asuransi)', 8, 'N', 'N', 59 FROM DUAL UNION ALL
            SELECT 'FBI_LAINNYA',                 'Lainnya (komisi notaris, denda tunggakan)',    8, 'N', 'N', 60 FROM DUAL UNION ALL
            -- GROUP 9: Direct OPEX
            SELECT 'OPEX_TOTAL',                  'Direct OPEX',                               9, 'Y', 'N', 61 FROM DUAL UNION ALL
            SELECT 'OPEX_MANPOWER',               'Manpower',                                  9, 'N', 'N', 62 FROM DUAL UNION ALL
            SELECT 'OPEX_TELECOM',                'IT & Telecommunication',                    9, 'N', 'N', 63 FROM DUAL UNION ALL
            SELECT 'OPEX_OFFICE_SUPPLIES',        'Office Supplies',                           9, 'N', 'N', 64 FROM DUAL UNION ALL
            SELECT 'OPEX_PERJALANAN_DINAS',       'Perjalanan Dinas',                          9, 'N', 'N', 65 FROM DUAL UNION ALL
            SELECT 'OPEX_SEWA',                   'Sewa (ATM, kendaraan, bangunan, dll.)',     9, 'N', 'N', 66 FROM DUAL UNION ALL
            SELECT 'OPEX_PREM_INS_NCR',           'Premium Insurance Non-Credit',              9, 'N', 'N', 67 FROM DUAL UNION ALL
            SELECT 'OPEX_PREM_AS_CR',             'Premi Asuransi Kredit',                     9, 'N', 'N', 68 FROM DUAL UNION ALL
            SELECT 'OPEX_TRAN_CR',                'Transaksi Kredit',                          9, 'N', 'N', 69 FROM DUAL UNION ALL
            SELECT 'OPEX_TRAN_NCR',               'Transaksi Non Kredit',                      9, 'N', 'N', 70 FROM DUAL UNION ALL
            -- GROUP 10: PPOP (Direct OPEX)
            SELECT 'PPOP_TOTAL',                  'PPOP (Dir OPEX)',                           10, 'Y', 'N', 71 FROM DUAL UNION ALL
            -- GROUP 11: Beban CKPN
            SELECT 'CKPN',                        'Beban CKPN',                                11, 'Y', 'N', 72 FROM DUAL UNION ALL
            -- GROUP 12: PBT (Dir OPEX + Retail CKPN)
            SELECT 'PBT_NOMINAL',                 'PBT (Dir OPEX + Retail CKPN)',              12, 'Y', 'N', 73 FROM DUAL
        ),
        cre AS (
            SELECT
                "kode_cabang",
